# FIXED LAB — VPC Lab 10: Network ACL Outbound Ephemeral Ports
# ─────────────────────────────────────────────────────────────
# Fix: Added outbound rule allowing ephemeral ports 1024-65535.
# This allows the web server to send return traffic back to
# clients on their randomly assigned ephemeral ports.
...
# ✅ THE FIX: Allow outbound ephemeral ports for return traffic.
# When a browser connects from a random source port (e.g. 54321),
# the server's response must go back out on that same port.
# NACLs are stateless — this must be explicitly allowed.
