...
exec > >(tee -a "$LOG") 2>&1
echo "=== Lab 02 Web Server Setup: $(date) ==="
...
<head>
  ...
  <title>AWS Broken Labs — Lab 02</title>
  <style>
...
<body>
  <div class="container">
    ...
    <h1>Lab 02 Fixed! ✅</h1>
    <h2>NACL rule ordering corrected — port 80 traffic now allowed.</h2>
    <div class="badge">VPC Lab 02 &mdash; Network ACL Rule Order</div>
...
[Unit]
Description=Broken Labs Lab 02 Web Server
...
systemctl is-active --quiet lab-web   && echo "=== ✅ Web server started ==="   || { echo "=== ❌ Web server failed ==="; journalctl -u lab-web -n 20; exit 1; }
